...
// windowsTokenizer is a tokenizer that does not treat the backslash as an
// escape character, so that Windows paths like C:\Users\me\editor.exe
// survive splitting.
type windowsTokenizer struct {
	DefaultTokenizer
}

func (t *windowsTokenizer) IsEscape(rune) bool {
	return false
}

// SplitWindows splits a string like Split with POSIX rules, except that the
// backslash is not an escape character.  Use it for command lines on Windows,
// where the backslash is a path separator.
func SplitWindows(s string) ([]string, error) {
	lexer := NewLexerString(s, true, true)
	lexer.SetTokenizer(&windowsTokenizer{})
